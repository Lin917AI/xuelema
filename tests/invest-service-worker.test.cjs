...
function worker() {
  ...
  const context = { URL, Request, Response, Promise,
    ...
    caches: { open: async () => ({ addAll: async (urls) => { stored.push(...urls); }, put: async () => {} }), keys: async () => ['xuelema-v12', 'xuelema-v13', 'xuelema-v14', 'another-app-v2'], delete: async (key) => { deleted.push(key); }, match: async (req) => new Response('cached:' + req.url) },
...
test('offline home and investment navigation use separate cached pages under Pages subpath', async () => {
  const w = worker();
  for (const [url, expected] of [['https://example.com/xuelema/?v=2.0', '/index.html?v=3.1'], ['https://example.com/xuelema/invest/', '/invest/index.html?v=3.1'], ['https://example.com/xuelema/invest/index.html', '/invest/index.html?v=3.1']]) {
    let response;
...
test('cache cleanup never deletes another project cache or IndexedDB', async () => {
  ...
  assert.deepEqual(w.deleted, ['xuelema-v12', 'xuelema-v13']);
});
test('all precache files exist and no private dataset enters public cache', async () => {
  ...
  assert.ok(w.stored.includes('./invest/data-store.js?v=3.1'));
